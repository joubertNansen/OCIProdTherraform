...
resource "oci_core_instance" "project_instance" {
  ...
  source_details {
       # source_type: Tipo de origem ("image" = usar uma imagem de SO)
    source_type = "image" # source_type: Tipo de origem ("image" = usar uma imagem de SO)
    image_id    = each.value.image_id # image_id: ID da imagem a usar (ex: imagem Ubuntu Linux, Windows Server, etc)
  }
